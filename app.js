...
app.get("/user/:id", checkSignIn, (req, res, next) => {
    let email = req.params.id.toLowerCase();
...
app.get("/", checkSignIn, (req, res) => {
    User.findOne({ email: req.session.user.toLowerCase() }, (err, user) => {
        if (user) {
...
app.post("/login", (req, res) => {
    ...
    User.findOne({ email: req.body.email.toLowerCase() }, (err, user) => {
        if (user == null) {
            ...
            res.render("index.ejs", {
                message:
                    "Doesn't look like you have an account! :( Please sign up.",
            });
        } else if (user) {
            bcrypt.compare(
                req.body.password,
            ...
            (err, user) => {
                if (err) {
                    ...
                    if (user.isPrivate == "private") {
                        ...
                        Post.create(
                            {
                                _id: user.publicPosts[publicPosts.length - 1]
                                    ._id,
                                author: user.email,
                                firstName: user.firstName,
                                lastName: user.lastName,
